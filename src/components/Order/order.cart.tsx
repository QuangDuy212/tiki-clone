...
const OrderPage = () => {
    ...
    useEffect(() => {
        if (carts)
            if (carts.length > 0) {
                const total = carts.reduce((total: number, item) => total + item.quantity * item.detail.price, 0);
                setTotalPrice(total);
            }
            else {
                setTotalPrice(0);
                                                        ...
                                                        <div className="product" >
                                                            <Row>
                                                                <Col className="product__img " xl={4} md={4} sm={12} xs={12}>
                                                                    <img src={`${process.env.VITE_BACKEND_URL}/images/book/${item.detail.thumbnail}`} />
                                                                </Col>
                    ...
                    {currentStep === 3 &&
                        <Col md={24} sm={24}>
                            <Result
                                ...
                                extra={<Button type="primary" onClick={() => router.push("/history")}>Lịch sử</Button>}
                            />
                        </Col>
                    }
                </Row>
